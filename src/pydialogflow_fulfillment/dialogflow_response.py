...
class DialogflowResponse:
    ...
    def add(self, dialog_response):
        ...
        elif isinstance(dialog_response, Card):
            self.google_payload["expectUserResponse"] = self.expect_user_response
            self.rich_response["items"].append(dialog_response.response)
        elif isinstance(dialog_response, BrowsingCarousel):
            fulfillment_message = OrderedDict()
            fulfillment_message["items"] = dialog_response.response
            carousel = OrderedDict()
            carousel["carouselBrowse"] = fulfillment_message
            self.rich_response["items"].append(carousel)
        elif isinstance(dialog_response, Carousel):
